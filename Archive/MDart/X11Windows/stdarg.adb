...
with Ada.Unchecked_Deallocation, Stdarg.Machine, System, System.Storage_Elements;
...
package body Stdarg is
   ...
   function Concat (Args : ArgList; Arg : T) return ArgList is
      ...
      Uncopied_bytes : Natural := (Nb_Int * Param_Size) - (T'Size / System.Storage_Unit);
      use System.Storage_Elements;
      Arg_Addr       : System.Address :=
         Arg'Address - (Arg'Address mod Storage_Offset (Int_Param_Alignment));
   ...
   begin
      if T_Is_Float then
         ...
         Long_Arg       := To_Long (Arg'Address, Arg'Size, Boolean'Pos (T_Is_Modular));
      ...
      case This_Arch is
         when I386 =>
            Buf_Addr := Args.Contents.Vector (Args.Contents.CurrentArgs + 1)'Address;
            Memcpy (Buf_Addr, Arg_Addr, (Nb_Int * Param_Size) - Uncopied_bytes);
         ...
         when HP =>
            Buf_Addr :=
              Args.Contents.Vector (MaxArguments - Args.Contents.CurrentArgs - Nb_Int + 1)'Address;
            if Arg_Size in 33 .. 64 then
               if Args.Contents.FirstHole = 0 then
                  Args.Contents.FirstHole := Args.Contents.CurrentArgs + 1;
               end if;
