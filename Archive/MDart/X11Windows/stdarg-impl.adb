...
package body Stdarg.Impl is
   ...
   function Address_of_Arg (Args : ArgList; Which : Which_Arg) return Param_Access is
   begin
   ...
   begin
      ...
      if This_Arch = Alpha then
         for Right_Index in 7 .. Right.Contents.CurrentArgs loop
            ...
            if Left_Index <= 12 then
               Left.Contents.Vector (Left_Index - 6) := Right.Contents.Vector (Right_Index - 6);
            end if;
            Left.Contents.Vector (Left_Index) := Right.Contents.Vector (Right_Index);
         end loop;
         Left.Contents.CurrentArgs := Left.Contents.CurrentArgs + Right.Contents.CurrentArgs - 6;
      ...
      for I in 1 .. Right.Contents.CurrentArgs loop
         if Right.Contents.FirstHole = I and then Left.Contents.CurrentArgs mod 2 /= 0 then
